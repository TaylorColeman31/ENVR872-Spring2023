...
#Get the data, which starts on line 30. Data was delimited by tabs, so we can use read.table in place of read_html()
...
URL_2 <- 'https://waterdata.usgs.gov/nwis/uv?search_site_no=02085070&period=24&format=rdb'

gage_2_data <- read.table(URL_2, skip = 29, header = T, sep = '\t', stringsAsFactors = T) #"skip" tells R to skip the first 29 rows and where to begin scraping data from

colnames(gage_2_data) = c("agency_cd", "site_no", "datetime", "tz_cd",
                        "discharge_cfs", "89192_00060_cd", "gage_ht_ft", "89193_00065_cd")

gage_2_data <- gage_2_data %>%
  select(datetime, discharge_cfs, gage_ht_ft) %>% 
  mutate(datetime = ymd_hm(datetime))

ggplot(gage_2_data, aes(x = datetime, y = discharge_cfs)) + 
  geom_line() +
...
#Compute annual means (we were given daily values)
neuse_by_year <- neuse %>% 
...
State_electricity <- data.frame("State" = the_states, 
                                "Price" = as.numeric(the_price), 
                                "Capacity" = as.numeric(gsub(",", "", the_capacity)), #gsub subs commas for blank space (removes them in our case)
...
total_retail <- the_website %>% 
  ...
  #Convert to dataframe and add column for year
...
df2017_2019 %>%
  ...
  ggplot() %>%
  geom_line(aes(x = year, y = Price, color = State)) +
