...
vector2
vector3 <- c(TRUE,TRUE,TRUE,FALSE,TRUE,FALSE) #logical vector (Booleans)
vector3

vector1[3] # Returns 3rd value = 5.3

# Matrices ----
matrix1 <- matrix(1:20, nrow = 5,ncol = 4) # 
matrix1
matrix2 <- matrix(1:20, nrow = 5, ncol = 4, byrow = TRUE) #byrow specifies order by rows (counts values across rows before moving down a column)
matrix2
...
matrix1[4, ] #values from row 4
matrix1[ , 3] #values from col 3
matrix1[c(12, 14)] #combining 12 and 14  
#Slicing data (:)
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
list1[[4]]
#semicolon allows us to combine 2 commands in one line of code

# Data Frames ----
d <- c(1, 2, 3, 4) # What type of vector? Numeric
e <- c("red", "white", "red", NA) # What type of vector? String
f <- c(TRUE, TRUE, TRUE, FALSE) # What type of vector? Binary vector
dataframe1 <- data.frame(d,e,f) #Combining the 3 vectors into 1 dataframe
...
> Answer:Data is interactive within Environment (can click on them to expand/open) whereas values are simply displayed

Question: In the R chunk below, write "dataframe1\$". Press `tab` after you type the dollar sign. What happens?

> Answer:The dollar sign allows you to extract from a column
...
```{r DataExercise-1a}
high_temps_F <- c(64, 71, 72, 63, 51, 52, 61, 64, 67, 73)

low_temps_F <- c(42, 49, 60, 46, 32, 32, 41, 47, 52, 54)

Fahrenheit <- c(high_temps_F, low_temps_F)
```
...
```{r DataExercise-1b}
high_temps_C <- (high_temps_F-32)*(5/9)
low_temps_C <- (low_temps_F-32)*(5/9)

#Creation of actual function:
convert_FtoC <- function(Fahrenheit){
  C <- round(((Fahrenheit)*5/9), 2)
return(C)
}
...
```{r DataExercise-1c}
Durham_forecast <- data.frame(high_temps_C = highsC, high_temps_F = highsF, low_temps_C = lowsC, low_temps_F = lowsF)
```
...
#sd command does not work on entire dataframe - only a specific column = vector!
sd(Durham_forecast)
sd(Durham_forecast$high_temps_C)
```
...
```{r DateExercise2}
#century issue - converting 2-digit dates to 4-digit dates
str_past <- "55-feb-3"
date_obj_past <- ymd(str_past)
date_obj_past

#extraction of year, removal of 2000 (%% 100) and addition of if/else statement: if last 2 digits of year of date_obj_past is above 23 (2023), then we want to remove 100 years (go from 2000 to 1900), returning the year.  
ifelse(year(date_obj_past) %% 100 > 23, year(date_obj_past) - 100, 
year(date_obj_past))
