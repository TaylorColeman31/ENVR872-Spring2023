...
# Set theme
mytheme <- theme_classic(base_size = 14) + # base_size sets font size
...
#Exercise 1:
# Plot 1
EPAair.O3.density.plot <- EPAair %>%
  ggplot(aes(x = Ozone, 
             fill = as.factor(Year)) + # we want R to interpret yr as a categorical variable 
  geom_density(alpha = .5) + # alpha function allows us to see overlay of both curves (sets transparency)
    scale_fill_manual(values = c("darkgreen", "darkblue")) +
    labs(x = "Ozone AQI value", y = "Density", fill = "")

  print(EPAair.O3.density.plot)

# Plot 2
EPAair.O3.boxplot <- EPAair %>%
ggplot(aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("b", "a")) +
labs(x = "", y = "Ozone AQI Value")

print(EPAair.O3.boxplot)

# Plot 3
EPAair.O3.violin <- EPAair %>%
  ggplot(aes(x = as.factor(Year), y = Ozone)) +
  geom_violin(draw_quantiles = 0.5) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 4, label = c("b", "a")) +
labs(x = "", y = "Ozone AQI Value") + 
  ylim(0,150) # adjusting y-axis to allow for "a" to be seen

print(EPAair.O3.violin)
...
#Exercise 2: Run a linear regression on PM2.5 by Ozone. Find the p-value and R-squared value. 
# PM2.5 = Y (dependent variable)
# O3 = X (independent variable)
PM25.O3.regression <- lm(EPAair$PM2.5 ~ EPAair$Ozone)
summary(PM25.O3.regression)

# Conclusion: Regression is significant. We can use O3 to predict PM 2.5. However, R-squared is very small and could be made stronger by adding more variables to the regression (see Exercise 4 on)!
...
# Get correlation and use to plot correlation matrix
PM25.O3.Corr <- cor(EPAair.subset)
corrplot.mixed(PM25.O3.Corr, upper = "ellipse")

#Exercise 5: Choose a model by AIC in a Stepwise Algorithm. Do the results from AIC match the variables you selected on Exercise 4? Ozone largest candidate (strongest positive), mean_Lat, and month (strongest negative correlation) appear to be potential additional predictors. Shape of ellipse denotes direction of correlation (+ or -) and fit to line (the narrower, the stronger)
...
# Conclusions: Year does not have a statistically significant influence on PM2.5
# Re-run lm() removing "Year" to see if R-squared improves from 0.13...
