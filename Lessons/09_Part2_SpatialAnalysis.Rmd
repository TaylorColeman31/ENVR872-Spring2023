...
#What is its CRS again? (EPSG = European Petroleum Survey Group)
...
# Read in USA counties shapefile and filter for NC 
# Don't need stringsAsFactors because dataset is already binary
...
#Reveal the extent of this dataset via the st_bbox() function. "st_" is a spatial command!
st_bbox(counties_sf)
# Confirms that we have a geographic coordinate system.
...
#Check the CRS
st_crs(huc8_sf)
# WGS84, just like counties dataset
...
# Approach 2: Mapping using ggplot
ggplot(data = huc8_sf) + 
...
mapview(huc8_sf, col.regions = "purple") +
  mapview(upperNeuse_sf, col.regions = "orange", alpha = 1)

#Approach 2: Creation of map using ggplot
ggplot(data = huc8_sf) +
  geom_sf(fill = "purple", color = "gray")+
  geom_sf(data = upperNeuse_sf, fill = "orange")
```
...
#Most are solar PV farms; let's remove those
nc_powerplants_sf <- nc_powerplants_sf %>% 
  filter(TYPE != "SOLAR PHOTOVOLTAIC") #"!=" for inequality
...
svi2018_nc_raw <- read.csv(
  ...
  colClasses = c('FIPS' = 'factor')) %>% #Tells R to keep FIPS column as a factor 
  select(COUNTY, FIPS, LOCATION, E_TOTPOP, E_POV, E_MINRTY)

#Check structure - no geometry!
str(svi2018_nc_raw)
...
#Produces same results as merge()

#View with mapview
mapview(counties_sf_join, 
        zcol = 'E_POV', 
        col.regions = brewer.pal(2, 'RdBu')) + 
  mapview(epa_pm25_sites_sf, cex = 'maxPM') #Sized by max PM values
...
wind_raw <- read.csv('https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv',
  colClasses = c('HUC_12' = 'factor')) %>%  #Explicitly sets the data to a factor format to treat HUC12 column values as characters
  mutate(HUC_8 = substr(HUC_12,1,8)) %>% #Creation of HUC8 column by taking first 8 digits from HUC12 code to get HUC8
  group_by(HUC_8) %>% 
...
#Select intersecting counties using matrix subsetting - Approach 1:
neuse_intersect_1 <- counties_sf_utm[neuse_sf,] #neuse_sf is our "selecting feature" whose geometry was used to select counties

#Select intersecting counties using the `filter()` command - Approach 2:
neuse_intersect_2 <- counties_sf_utm %>% 
  st_filter(neuse_sf, .pred = st_intersects) #Default predicate is st_intersects (can also use st_is_within_distance, st_nearest_feature, st_equals, st_overlaps, etc.)
...
neuse_counties_sf <- neuse_counties_sf %>% 
  mutate(Area_m2 = as.numeric(st_area(neuse_counties_sf$geometry))) #Have to specify "as.numeric," otherwise, new Area_m2 column simply adds ("m^2") to preexisting values
...
the_triangle <- counties_sf_utm %>%
  filter(NAME == "Chatham"| NAME == "Durham" | NAME == "Wake" | NAME == "Orange")
#OR filter(NAME %in% c("Chatham", "Durham", "Wake", "Orange"))
  
#Grab the intersecting HUC_8s that reside within the Triangle counties. Returns all rows from huc8_sf_utm that intersect with the_triangle's shape 
...
#Combine the centroids into one feature and construct a convex hull (= "chull") around them
...
#Combine the centroids into one feature and draw voronoi polygons. Voronoi polygons define the area that is closest to a given point (in this case, our centroids)
...
#User coordinates for Grainger Hall
userLat = 36.0045442
userLng = -78.9426381

#Create a simple features point geometry from the point - coordinates are now known
theSite_sfp <- st_point(c(userLng,userLat))

#Create a simple features column from the point geometry object - coordinates now have a reference on Earth from being projected
...
sync(m1,m2)
#Can save as a web page and email to others using the Export button in Viewer!
```
