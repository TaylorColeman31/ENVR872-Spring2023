...
```{r continuous variables over time, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()
# Method 2: Create an object to save all ozone plots to = recommended method!
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone))
print(O3plot)

# Incorrect way to change color of dots:
...
# How could you automatically assign a marker color to a variable? Don't need to worry about this; ggplot automatically does it for you! 
# Assigning a shape based on the column "Year" and different color for each site name
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# Separate plot with facets to see differences between each year better
PMplot.faceted <-
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year) #Creates grid-like view for plots, with Site.Name as rows and Year as columns
...
```{r visualizing relationship between 2 continuous variables, fig.height = 3, fig.width = 4, warning = FALSE}
# Scatterplot to see relationship between "irradianceWater" and DO2
lightvsDO <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point()
print(lightvsDO)

# Adjust axes to see largest concentration of data (and ignore extreme outliers)
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
#Plotting TN vs. TP, coloring points by depth, with a trendline and specified line (if you know the slope and intercept of trendline)
NvsP <-
...
```{r continuous and categorical variables, fig.height = 3, fig.width = 4, warning = FALSE}
# Box and whiskers plot
Nutrientplot3 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? "Fill" would color each box only, not the data points
print(Nutrientplot3)
# Hard to make assumptions for first few nutrients in Paul Lake due to flattened boxplot...

# Dot plot - "binaxis" command stacks observations based on y-axis; "position = 'dodge'" does not allow for overlap of dots
Nutrientplot4 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Violin plot - adds density information to summary statistics!
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Frequency polygons
# Using a tidy dataset ("PeterPaul.chem.nutrients")
Nutrientplot6 <-
