...
```{r themes, fig.height = 3, fig.width = 4, warning = FALSE}
O3plot <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) 
print(O3plot)

# Default theme for ggplot: gray background with white lines
O3plot1 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_gray()
print(O3plot1)

# Changing theme to "bw" theme
...
```{r custom theme, fig.height = 3, fig.width = 4, warning = FALSE}
# Define "mytheme"
mytheme <- theme_classic(base_size = 14) +
  theme(axis.text = element_text(color = "black"), 
        legend.position = "top") #alternative: legend.position + legend.justification (changes alignment of legend i.e., right, center, etc.)
...
# Option 2: Setting theme at the start/Setting a theme for all subsequent plots
theme_set(mytheme)
...
```{r changing multiple plot components, fig.height = 3, fig.width = 4, warning = FALSE}
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + #"lty" specifies dashed line type for horizontal line
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + # Default shape = circle
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") +   # Adding "good" label: "hjust" = "1" means left alignment for text
  geom_text(x = as.Date("2020-01-01"), y = 95, label = "moderate", hjust = 1, fontface = "bold") + # Adding "moderate" data label
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + #Adding "unhealthy (sensitive groups)" label
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), # Plotting only a section of data between these dates
    date_breaks = "2 months", date_labels = "%b %y") + 
  ylab(expression("O"[3]* " AQI Value")) + # Changes y-axis label to something more informative
...
# Step 1: change your axis labels to reflect TN and TP in micrograms per liter.
# Step 2: change your legend labels
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
...
```{r multiple plots on 1 pg, fig.height = 7, fig.width = 6}
