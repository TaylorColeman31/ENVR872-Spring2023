...
> Answer: A message was recieved for the loading of tidyverse version 1.3.2 that included "attaching packages" (tibble, purrr, tidyr, readr, forcats, and stringr) and "conflicts"
...
* *file = * : use this input to point to your data file. If it's on the same folder as your .Rmd then you only need to write the file name. But if it's in another folder you need to point to the path were file is located;
* *header =* : if your file has a header you should set this to TRUE, o.w. FALSE;
* *skip =* : if your file has rows explaining the data or any other rows on the top that need to be skipped you should just set skip to be equal to the number of row that should be skipped before reading the data. Note that if header=TRUE, you should not skip the row with the header. The defaul is *skip=0*;
* *dec =* : use to define decimals *dec="."* or *dec=","* (if data is international) depending on how it's defined on your set. The default is ".".
...
# Answer: Our Environment will remain blank!
...
> ANSWER: You can find this information within the "Metadata" folder. The placement and information found in this file provides a means of understanding the derivation/origin of the data, the units that it is represented in, and ways to learn more about it (through the provisioning of additional resources).
...
> Answer: They are replaced with "NA" 
...
> Answer: "NAs" are never removed for time series analysis; however, they might be replaced by another value. Any other form of analysis might require the removal of these missing values to focus on the ones available to perform calculations, etc.   
...
```{r Saving data}
write.csv(USGS.flow.data, file = "./home/guest/EDA-Fall2022/Data/Processed/USGS_Site02085000_Flow_Processed.csv", row.names=FALSE)
