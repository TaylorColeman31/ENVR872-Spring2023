...
# Checking all values of plotID
summary(Litter.Totals$plotID)
# Not a balanced design; however, we can still run the 1-way ANOVA test

# Testing assumption 1: Test for normality. 
...
# Plot ID "NIWO_040" does not follow normal dist.
...
# Another way to test for normality:
qqnorm(Litter.Totals$dryMass); qqline(Litter.Totals$dryMass)
# To be normal, data points would need to follow the qqline. This is not the case for dry mass

# Testing assumption 2: Test for equal variance
...
#results: reject null hypothesis i.e. difference between a pair of group means is statistically significant. Means are not all the same! But how can we know which means are different? Need to run a post-hoc test!

# Side note: Can run ANOVA as lm function
# Summary from lm linear regression contains one row for each level of categorical variable.
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
# Note that "NIWO_040" = "Intercept" term, and all other estimates are relevant to this intercept term. For example, "NIWO_041" has a value of 1.299 = (1.299 + 15.580)
...
# Provides diff = difference between each pair of means, lwr and upr for lower and upper CI values, and p-value for multiple comparisons

# Extract groupings for pairwise relationships for grouping pairs of similar means
Litter.Totals.groups <- HSD.test(Litter.Totals.anova, "plotID", group = TRUE)
Litter.Totals.groups
# Groups of similar means classified by shared letters

# Graph the results
Litter.Totals.plot <- ggplot(Litter.Totals, aes(x = plotID, y = dryMass)) +
  geom_boxplot() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  stat_summary(geom = "text", fun = max, vjust = -1, size = 3.5, #vjust angles text for plot ID labels
...
> All sites with "a" have similar means. To report this, it would help to reorder the boxplots by sites/plot IDs, keeping all "a's" together. 
...
# Conclusion: We can reject H0 (litter masses do seem to vary across functional groups and land cover classes)

# Format as lm
Litter.anova.2way2 <- lm(data = Litter, dryMass ~ functionalGroup + nlcdClass)
summary(Litter.anova.2way2)
# Same conclusion from aov

# Run a post-hoc test for pairwise differences - USE TO DETERMINE SIGNIFICANT INTERACTIONS TO REPORT WITH FINDINGS
TukeyHSD(Litter.anova.2way)

# Conclusion: Any functional group pairs that have a "p adj" value < 0.05 should be included as a predictor interaction variable for dry masses. For example, needles and flowers and needles AND flowers combined should be included in the model/considered!
...
```{r considering interaction effects between land cover and functional group}
# Format as aov
...
# Use to view the merge between functional groups and NLCD classes in table format:
# Litter.test <- cbind(Litter,Litter.interaction)

# Comparing means for each litter interaction level (24 levels total)
Litter.anova.2way5 <- aov(data = Litter, dryMass ~ Litter.interaction)

# Grouping dataset based on interaction levels
