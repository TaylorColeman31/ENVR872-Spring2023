...
> Add notes here: To monitor the water quality and overall health of a lake!
...
# To compare one value against another; equality sign (note that '=' is an assignment operator!):
`==`
# Inequality sign:
`!=`
# Less than: 
...
`>`
`>=`
# Use '&' to check multiple conditions in which both conditions must be true
`&`
# Use '|' to check mutliple conditions in which one or the other condition must be true
`|`
...
# matrix filtering - filtering only those rows featuring a 0 value for the "depth" column  
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering - use of filter function to achieve the same result from filtering the matrix
...
# Selection of both lakes to analyze using "|" as "or" to return observations from one lake or the other
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")
# Option 2: The opposite of selecting the lakes we want; excluding all other lakes
...
# Option 3: Cleaner way to filter dataset asking R to filter for occurrence of the names "Paul Lake" and "Peter Lake" within "lakename" vector
...
# Filtering data for range of values between June and October
# Recommendation: Open Excel and look at data values to determine the "daynum" range that fits our desired date range! daynum 151 = May 31; 305 = Nov 1
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305)
# Option 2: Comma can be used within the expression to function in the same way as "&"
...
# Answer: "year4" is an integer; therefore, we can use the same technique that we did with "daynum"!
...
```{r selecting columns}
# Analogous to creating a tidy subset of the original dataset
...
```{r mutate}
# Creating new column for temp in F
...
```{r piping}
# Provides the same output as NTL.phys.data.temps (but fewer observations due to filter for Peter and Paul lakes)
...
# Adding PeterPaul processed dataset to "Processed" folder within the "Data" folder in my cloned repository
...
# Answer: To keep our data tidy and not interfere with the steps taken to process the data (in case we need to update it/come back to it later on)
