...
author: Environmental Data Analytics | Taylor Coleman
date: "Spring 2023"
output:
 pdf_document: 
  toc: TRUE
...
-   R Markdown has many formats for output:

    -   Documents

    -   Presentations

    -   R Shiny Widgets and Dashboards

    -   From 3rd party templates
...
#NOTE: install() packages should NOT be used with files that are to be knitted - it results in errors! 

```

### Setup

Create an R chunk below called "setup" that loads the packages `tidyverse`, `lubridate`, `here`, and `knitr` and sets a ggplot theme. Remember that you need to disable R from throwing a message, which contains a check mark that cannot be knitted.
