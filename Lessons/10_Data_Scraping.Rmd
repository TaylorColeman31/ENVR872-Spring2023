...
df_withdrawals <- data.frame("Month" = rep(1:12), #Creates a series of numbers for month column
                             "Year" = rep(2000,12), #"rep" = repeat
...
df_withdrawals <- df_withdrawals %>% 
  mutate(Registrant = !!the_registrant, #!! = "bang bang"; tells R to set the data to a variable and not a string
...
the_website <- read_html(the_scrape_url) #"read_html" = a "hook" to the website url
...
scrape.it <- function(the_year, the_facility){
  ...
  #When iteratating a function, we use sys.sleep(1) to add a pause between each iteration to prevent a website provider from cutting off our access to a dataset and not be mistaken for a security attack.
...
ggplot(the_df,aes(x=Date,y=Avg_Withdrawals_mgd,color=Facility_name)) + 
  #geom_line() + 
  geom_smooth(method="loess",se=FALSE) + #specifies loess regression and no standard error bars
