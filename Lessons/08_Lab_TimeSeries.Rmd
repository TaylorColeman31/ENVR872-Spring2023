...
is.na(BR_complete$Temperature_C)
...
ggplot(BR_complete,
  ...
  geom_line()+
  geom_smooth(method = "lm")
#Relatively constant trend with seasonality (up and down movements)
...
```{r}
#Exercise 7: Create a time series object for each series using ts(). Make sure you specify the starting point and frequency.
...
#Option 2: Rainfall_ts <- ts(BR_complete$Rainfall, start = c(1901, 1), frequency = 12)
...
Rain_components <- as.data.frame(Rainfall_decomp$time.series[,1:3])
...
#Extracting Date and Rainfall observations from original dataset to be added to components data
...
ggplot(Rain_components, aes(x = Date)) +
...
Temp_trend <- trend::smk.test(Temp_ts)
summary(Temp_trend)
#All p-values for temperature are less than 0.05, allowing us to reject the null hypothesis that the temperatures are independent, identically distributed. Additionally, positive S values indicate an increasing trend.
