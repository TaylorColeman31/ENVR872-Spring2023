...
```{r plotting TSA}
#Always begin TSA by plotting variables!
...
#Error/random component = trickiest thing to detect in TSA!
...
#Top panel of plot = observed data, 2nd panel from top = seasonal component, 3rd panel = trend component, and bottom panel = remainder random component
#Conclusion: We do not have a monotonic trend (there are increasing and decreasing trends)!
...
#Adding discharge_mean and datetime variables to EnoDischarge
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_line(aes(y = trend, x = Date), color = "#c13d75ff") + #color given to make trend line stand out
...
```{r interpolation of wind speed data}
head(wind_data)
...
wind_data_clean <- 
  wind_data %>% 
  mutate( AWND.clean = zoo::na.approx(AWND) ) #calling na.approx function from zoo package

summary(wind_data_clean$AWND.clean)
#Note the NA is gone

#Now we have a necessary continuous dataset for trend analysis and TSA!
...
# Super important for forecasting, or fitting a trend to a dataset
...
#Approach 1: Run SMK test (because there is seasonality) using SeasonalMannKendall()
wind_data_trend1 <- Kendall::SeasonalMannKendall(wind_data_ts)

# Inspect results
wind_data_trend1
summary(wind_data_trend1)
# p-value > 0.05; we fail to reject H0 (that variables are independent, identically distributed). Trend seems to be stationary. 

# Approach #2: Run SMK test using smk.test()
wind_data_trend2 <- trend::smk.test(wind_data_ts)
# Inspect results
wind_data_trend2
summary(wind_data_trend2)
#Returns S, tau, and p-values for each season. Confirms that there is an overall decreasing trend. Where p-value > 0.05, trend is stationary
...
> Answer: The trend for wind speed is decreasing over time.
