...
PeterPaul.chem.nutrients <- read.csv(
  ...
  stringsAsFactors = TRUE) #here is more stable than setwd() to point to project directory
...
#Need to convert month to a factor -- with 12 levels, labeled with month names
factor(PeterPaul.chem.nutrients$month,
       levels = 1:12)

#Try again: Plot how many Lake observations occur each month. "drop = FALSE" maintains empty categories on x-axis
ggplot(PeterPaul.chem.nutrients,
...
#Apply a ggtheme ("wsj" = Wall Street Journal; theme_excel is another theme)
surface_temp_1999_plot + 
  theme_wsj() + 
  scale_color_wsj()
...
# 1. 
# Plot surface temperatures by day of year. Day of year = independent variable that should be placed on x-axis!
# Color your points by year, and facet by lake in two rows.
# Change the ylab name
surface_temp_by_day <- PeterPaul.chem.nutrients %>%
ggplot(aes
       (x = daynum, 
         y = temperature_C)) + 
  ...
  labs(
    title = "Surface Temperatures for Peter and Paul Lake by Day",
    x = "Sample Date",
    y = "Surface Temperature in °C")

#2. 
# Plot temperature by date. Color your points by depth. Date = independent variable that goes on x-axis!
