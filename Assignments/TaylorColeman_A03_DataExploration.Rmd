...
#Having R return the number of rows and the number of columns for the Neonics dataset. The dimensions of this dataset are 4,632 rows x 30 columns.
dim(Neonics)
```
...
sort(Neonics$Species.Common.Name, decreasing = TRUE)
```
...
#Constructing a frequency line graph to display studies conducted by publication year from the Neonics dataset
ggplot(Neonics) + 
...
#Determining the class of collectDate from the Litter dataset. It is not recognized by R as a date (it is a "factor")!
...
#Cconfirming that collectDate is now recognized as a "date"!
class(collectDate)
```
